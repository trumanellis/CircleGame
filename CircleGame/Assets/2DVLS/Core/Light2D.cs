...
//public delegate void LightEventHandler(Light2D light);
...
public class Light2D : MonoBehaviour {
    ...
    private static event Light2DEvent OnLightEnter;
    public static event Light2DEvent OnLightEnterEvent {
        add { OnLightEnter += value; }
        remove { OnLightEnter -= value; }
    }

    private static event Light2DEvent OnLightStay;
    public static event Light2DEvent OnLightStayEvent {
        add { OnLightStay += value; }
        remove { OnLightStay -= value; }
    }

    private static event Light2DEvent OnLightExit;
    public static event Light2DEvent OnLightExitEvent {
        add { OnLightExit += value; }
        remove { OnLightExit -= value; }
    }
        ...
        if(gameObject.activeSelf) {
            ...
            if(useEvents) {
                for(int i = 0; i < identifiedObjects.Count; i++) {
                    if(OnLightExit != null)
                        OnLightExit(this, identifiedObjects[i]);

                    identifiedObjects[i].SendMessage("OnVLightExit", this, SendMessageOptions.DontRequireReceiver);
        ...
        if(Application.isPlaying && useEvents) {
            for(int i = 0; i < unidentifiedObjects.Count; i++) {
                if(identifiedObjects.Contains(unidentifiedObjects[i])) {
                    if(OnLightStay != null)
                        OnLightStay(this, unidentifiedObjects[i]);
                    unidentifiedObjects[i].SendMessage("OnVLightStay", this, SendMessageOptions.DontRequireReceiver);
                }

                if(!identifiedObjects.Contains(unidentifiedObjects[i])) {
                    identifiedObjects.Add(unidentifiedObjects[i]);

                    if(OnLightEnter != null)
                        OnLightEnter(this, unidentifiedObjects[i]);
                    unidentifiedObjects[i].SendMessage("OnVLightEnter", this, SendMessageOptions.DontRequireReceiver);
                }
            }
    ...
    public void TriggerBeamEvent(LightEventListenerType eventType, GameObject eventGameObject) {
        switch(eventType) {
            case LightEventListenerType.OnEnter:
                if(OnLightEnter != null)
                    OnLightEnter(this, eventGameObject);
                eventGameObject.SendMessage("OnVLightEnter", this, SendMessageOptions.DontRequireReceiver);
                break;

            case LightEventListenerType.OnStay:
                if(OnLightStay != null)
                    OnLightStay(this, eventGameObject);
                eventGameObject.SendMessage("OnVLightStay", this, SendMessageOptions.DontRequireReceiver);
                break;

            case LightEventListenerType.OnExit:
                if(OnLightExit != null)
                    OnLightExit(this, eventGameObject);
                eventGameObject.SendMessage("OnVLightExit", this, SendMessageOptions.DontRequireReceiver);
                break;
        }
    }
